...
\begin{enumerate}
  ...
  \item Le script \texttt{analyze\_samples.py} peut appeler l'API, generer \texttt{result.json} et ouvrir \texttt{result\_report.html}.
\end{enumerate}
...
Donc la base vectorielle sert pendant l'analyse, mais elle ne garde pas les resultats pour biaiser le test suivant. Les fichiers \texttt{result.json} et \texttt{result\_report.html}, eux, sont seulement des rapports ecrases ou regeneres par script.
...
\texttt{NVIDIA\_TEMPERATURE=0.1} reduit la variabilite de generation du LLM. Cela ne garantit pas une identite parfaite a 100\%, car un service LLM peut rester legerement non deterministe, mais cela rend les sorties beaucoup plus stables.
...
Ce dossier contient des sous-dossiers prevus pour stocker des documents ou des resultats. Dans la logique actuelle, le classement principal est retourne par l'API et les rapports de test sont generes a la racine du backend sous forme de \texttt{result.json} et \texttt{result\_report.html}.
...
\subsection{\texttt{analyze\_samples.py}}

Script de test complet. Il lit \texttt{samples/fiche\_poste.pdf}, recupere les CV du dossier, appelle l'API FastAPI, sauvegarde le JSON et ouvre le rapport HTML. Il accepte aussi \texttt{--cv-file} et \texttt{--cv-dir} pour tester d'autres fichiers.

\subsection{\texttt{render\_result\_report.py}}

Convertit \texttt{result.json} en rapport HTML lisible. Il affiche:
