...
Abstraction du stockage vectoriel dans PostgreSQL avec extension pgvector. La configuration locale utilise le conteneur Docker \texttt{smartrecruit-db} (\texttt{pgvector/pgvector:pg16}) publié sur \texttt{127.0.0.1:5433}. Le store gère le stockage des embeddings, la recherche par similarité vectorielle et le nettoyage des namespaces après analyse.
...
\begin{itemize}
    ...
    \item \textbf{Opérateur <>} : Utilise la similarité cosinus native de pgvector. La colonne \texttt{embedding} est en \texttt{vector(2048)}, dimension du modèle NVIDIA \texttt{nvidia/llama-nemotron-embed-1b-v2}.

    \item \textbf{Index vectoriel} : Aucun index HNSW n'est créé actuellement, car pgvector limite les index HNSW sur le type \texttt{vector} à 2000 dimensions alors que le modèle actif retourne 2048 dimensions. Un index approximatif demanderait \texttt{halfvec} ou une projection dimensionnelle dans une migration dédiée.
...
def ensure_cv_quota(cv_files):
    """Vérifie uniquement qu'au moins un CV est fourni"""
...
\begin{description}
    ...
    \begin{itemize}
        ...
        \item text, vector\_json, embedding (\texttt{vector(2048)} pgvector)
    \end{itemize}
\end{description}
...
\begin{itemize}
    ...
    \item \textbf{Performance} : Recherche vectorielle native via l'opérateur cosinus pgvector \texttt{<=>}; l'index HNSW n'est pas activé avec le modèle 2048 dimensions actuel.
    \item \textbf{Flexibilité} : Peut aussi stocker les résultats d'analyses
    \item \textbf{Configuration active} : \texttt{VECTOR\_BACKEND=pgvector}. Le mode JSON est une option explicite pour certains usages de développement local.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Versioning des analyses} : Associer chaque analyse a une version de code identifiable
\end{itemize}
