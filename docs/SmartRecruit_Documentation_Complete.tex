...
Abstraction du stockage vectoriel dans PostgreSQL avec extension pgvector. La configuration locale vérifiée utilise le conteneur Docker \texttt{smartrecruit-db} (\texttt{pgvector/pgvector:pg16}) publié sur \texttt{127.0.0.1:5433}. Le store gère le stockage des embeddings, la recherche par similarité vectorielle et le nettoyage des namespaces après analyse.
...
\begin{itemize}
    ...
    \item \textbf{Opérateur <>} : Utilise la similarité cosinus native de pgvector. La colonne \texttt{embedding} est en \texttt{vector(2048)}, dimension vérifiée avec le modèle NVIDIA \texttt{nvidia/llama-nemotron-embed-1b-v2}.
...
\begin{itemize}
    ...
    \item \textbf{Configuration active} : \texttt{VECTOR\_BACKEND=pgvector}. Le mode JSON reste uniquement un choix explicite legacy/dev et n'est pas utilisé par la configuration locale vérifiée.
\end{itemize}
