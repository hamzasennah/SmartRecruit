...
\author{Projet PFE -- Backend FastAPI, RAG, NVIDIA API et PostgreSQL}
\date{\today}
...
\begin{itemize}
    ...
    \item recherche RAG avec embeddings NVIDIA et stockage vectoriel PostgreSQL ;
...
Le backend utilise NVIDIA API via un endpoint compatible OpenAI. Les extracteurs appellent le modèle configuré pour produire un JSON structuré, puis le backend valide et normalise ce JSON avant le scoring.
...
Les sections des CV sont découpées en chunks. Chaque chunk est transformé en vecteur par le client d'embeddings NVIDIA. Ces vecteurs sont stockés dans PostgreSQL avec SQLAlchemy.
...
Cette requête est vectorisée, puis utilisée pour récupérer les passages les plus pertinents du CV dans PostgreSQL par similarité cosinus entre vecteurs NVIDIA.
...
\hline
\texttt{backend/app/infrastructure} & Contient les clients techniques vers NVIDIA API et PostgreSQL : LLM, embeddings et stockage vectoriel SQLAlchemy. \\
\hline
...
\hline
\texttt{backend/scripts} & Contient les scripts d'aide pour lancer ou préparer l'environnement : test NVIDIA API, PostgreSQL et backend. \\
\hline
...
\hline
\texttt{backend/app/config.py} & Centralise la configuration : chemins, clé et URL NVIDIA, modèles NVIDIA, PostgreSQL et limite d'upload. \\
\hline
\texttt{backend/app/dependencies.py} & Assemble les dépendances principales : parser, client LLM NVIDIA, client embeddings NVIDIA, stockage vectoriel PostgreSQL et pipeline de classement. \\
\hline
\end{longtable}
...
\hline
\endhead
\texttt{infrastructure/nvidia\_llm.py} & Client LLM NVIDIA via endpoint compatible OpenAI. Utilisé pour générer du JSON structuré. \\
\hline
\texttt{infrastructure/nvidia\_embeddings.py} & Client d'embeddings NVIDIA. Transforme les chunks et requêtes en vecteurs. \\
\hline
