...
\author{Projet PFE -- Backend FastAPI, RAG, Qwen et Qdrant}
\date{\today}
...
\begin{itemize}
    ...
    \item recherche RAG avec embeddings Qwen et base vectorielle Qdrant ;
...
Le backend peut utiliser un LLM Qwen via un endpoint compatible OpenAI. En mode test, il utilise une extraction heuristique locale afin que les tests ne dépendent pas d'un serveur externe.
...
Les sections des CV sont découpées en chunks. Chaque chunk est transformé en vecteur par le fournisseur d'embeddings Qwen. Ces vecteurs sont stockés dans Qdrant.

En mode test, un fournisseur d'embeddings déterministe est utilisé pour obtenir des résultats reproductibles.
...
Cette requête est vectorisée, puis utilisée pour récupérer les passages les plus pertinents du CV dans Qdrant. Les résultats sont ensuite réordonnés par le reranker.
...
\hline
\texttt{backend/app/database} & Prépare la couche base de données relationnelle. Elle est prévue pour la persistance future des analyses, documents ou candidats. \\
\hline
\texttt{backend/app/providers} & Contient les connecteurs vers les composants externes : LLM, embeddings, reranker, base vectorielle, base SQL. \\
\hline
\texttt{backend/app/repositories} & Contient les classes d'accès aux données persistantes. Ces fichiers préparent une architecture propre si une base relationnelle complète est activée. \\
\hline
...
\hline
\texttt{backend/scripts} & Contient les scripts d'aide pour lancer ou préparer l'environnement : GPU, modèles, bases, serveur vLLM. \\
\hline
...
\hline
\endhead
\texttt{backend/requirements.txt} & Liste les dépendances Python nécessaires : FastAPI, Uvicorn, PyMuPDF, Qdrant, SQLAlchemy, pytest, etc. \\
\hline
\texttt{backend/docker-compose.yml} & Prépare les services externes locaux comme Qdrant et PostgreSQL. \\
\hline
\texttt{backend/app/main.py} & Point d'entrée FastAPI. Il crée l'application, configure CORS et inclut les routes. \\
\hline
\texttt{backend/app/config.py} & Centralise la configuration : chemins, URL des providers, modèles Qwen, collection Qdrant, mode test. \\
\hline
\texttt{backend/app/dependencies.py} & Assemble les dépendances principales : parser, LLM, embeddings, vector store, reranker et pipeline de classement. \\
\hline
\end{longtable}
...
\hline
\texttt{retrieval/section\_indexer.py} & Transforme les chunks en embeddings puis les insère dans la base vectorielle. \\
\hline
\texttt{retrieval/semantic\_retriever.py} & Recherche les passages pertinents dans la base vectorielle à partir de la requête issue de la fiche de poste. \\
\hline
\texttt{retrieval/reranking\_pipeline.py} & Trie les résultats récupérés selon leur pertinence. \\
\hline
\end{longtable}
...
\hline
\endhead
\texttt{providers/llm/qwen\_vllm\_provider.py} & Fournisseur LLM Qwen via endpoint compatible OpenAI. Utilisé pour générer du JSON structuré. \\
\hline
\texttt{providers/embeddings/qwen\_embedding\_provider.py} & Fournisseur d'embeddings Qwen. En mode test, utilise un embedding déterministe local. \\
\hline
\texttt{providers/reranker/qwen\_reranker\_provider.py} & Réordonne les passages récupérés selon leur proximité avec la requête. \\
\hline
\texttt{providers/vector\_store/qdrant\_provider.py} & Connecte le backend à Qdrant. En mode test, utilise un vector store en mémoire. \\
\hline
\texttt{providers/database/postgres\_provider.py} & Prépare la connexion PostgreSQL. \\
\hline
\end{longtable}
...
\hline
\endhead
\texttt{tests/conftest.py} & Configure l'environnement de test, notamment le mode test. \\
\hline
